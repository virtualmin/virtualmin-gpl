...
# with_locked_domain(&domain|id, &callback)
# Lock and reread one domain, then pass its current record to the callback.
# Keep the lock through the callback. Release it on completion or failure only
# if acquired here; leave any lock already held by the caller in place.
# Return the callback's scalar result, or undef if the domain cannot be read.
# The callback must save its changes. The supplied object is left untouched.
# Do not start another update to the same domain inside the callback.
sub with_locked_domain
{
my ($id, $code) = @_;
$id = $id->{'id'} if (ref($id));
# A second reread would discard changes pending in the outer update.
&error("Domain $id is already being updated!") if ($domain_lock_scope{$id});
...
my ($result, $err);
{
	# Feature helpers may unlock the same domain. Only this scope may release it.
	local $domain_lock_scope{$id} = $$;
	# Forced reads otherwise overwrite the caller's cached hash in place.
	local $main::get_domain_cache{$id};
...
# Clear the restored cache so the next lookup reads the saved record.
...
return $err if (!&with_locked_domain($domain, $prepare));
# Run the before command
...
if (&has_group_quotas()) {
	...
	my $bsize = &quota_bsize("home");
	my ($homequota) = &get_domain_quota($d);
...
if (!$d->{'alias'} && $config{'dkim_enabled'} && &has_dkim_domain($d)) {
...
if ($itype =~ /^(rpm|deb)$/ &&
	...
	my $date_trigger_warning = 1798761599;        # December 31, 2026
...
# domain_owner_module_registry()
# Returns all domain-owner module definitions and their backing Webmin modules.
sub domain_owner_module_registry
{
&require_mysql();
...
# Returns installed modules that can be granted to domain owners. This stable
# registry is used when building or storing access policies, so it must never
...
foreach my $m (@$modules) {
	...
	push(@available, $m) if (&foreign_check($fmod));
	}
return @available;
}

# list_available_domain_owner_modules()
# Returns the installed domain-owner modules accessible to the calling Webmin
...
foreach my $m (@$modules) {
	...
	push(@available, $m) if (&foreign_available($fmod));
	}
return @available;
}
...
if ($tmpl && ($tmpl->{'id'} == 1 || !$tmpl->{'for_parent'})) {
	...
	@rv = grep { $_ ne 'resources' && $_ ne 'unix' && $_ ne 'webmin' } @rv;
	}
my @rvdesc = map { [ $_,
...
# get_dns_ip([reseller-name-list], [prefer-protocol])
# Returns the IP address for use in DNS records, or undef to use the domain's IP
sub get_dns_ip
{
my ($reselname, $prefer) = @_;
$prefer ||= 4;
my $suffix = $prefer == 6 ? 6 : "";
if (defined(&get_reseller)) {
...
foreach my $d (&list_domains()) {
	&with_locked_domain($d, sub {
		...
		if ($logins[0] > ($d->{'last_login_timestamp'} || 0)) {
