...
=head1 configure-scripts.pl

Configure multiple web app scripts

This command can be used to modify settings, perform backups, create clones,
and carry out other administrative tasks for multiple web app scripts at once,
if supported by the applications.
...
# Locate the mass usage and CLI handlers for this script type
my $script_usage_func = &script_find_kit_func($web_app_name, 'usage_mass');
my $script_cli        = &script_find_kit_func($web_app_name, 'cli_mass');
...
{
print "$_[0]\n\n" if ($_[0]);
print "Configure multiple web app scripts\n\n";
print "virtualmin configure-scripts --script-type name";
