...
if (!$force && $user ne $mysql::config{'login'}) {
	&usage("Warning! Changing virtual server password here is not going to update Virtualmin internal database, which is going to result in a connection error for some operations in the future. It is highly recommended to change virtual server database password using special \`virtualmin modify-database-pass\` command instead.\n");
	}
...
print "                         [--user username]\n";
print "                         [--force password change for virtual server]\n";
exit(1);
}
